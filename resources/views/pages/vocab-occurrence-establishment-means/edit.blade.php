...
@section('page_title','Editar — Vocab occurrence establishmentmeans')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab occurrence establishmentmeans #{ $item->estabmeans_id }</h1>
...
@endif
<form method="POST" action="{{ route('vocab-occurrence-establishment-means.update', $item) }}" class="card card-body">
  @csrf @method('PUT')
