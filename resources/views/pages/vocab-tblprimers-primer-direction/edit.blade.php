...
@section('page_title','Editar — Vocab tblprimers primerdirection')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab tblprimers primerdirection #{ $item->id_primerdirection }</h1>
...
<form method="POST" action="{{ route('vocab-tblprimers-primer-direction.update', $item) }}" class="card card-body">
  @csrf @method('PUT')
