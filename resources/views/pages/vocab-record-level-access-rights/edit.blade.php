...
@section('page_title','Editar — Vocab record level accessrights')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab record level accessrights #{ $item->accessrights_id }</h1>
...
<form method="POST" action="{{ route('vocab-record-level-access-rights.update', $item) }}" class="card card-body">
  @csrf @method('PUT')
