...
@section('page_title','Editar — Vocab record level collectioncode')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab record level collectioncode #{ $item->collectionCode_id }</h1>
...
<form method="POST" action="{{ route('vocab-record-level-collection-code.update', $item) }}" class="card card-body">
  @csrf @method('PUT')
