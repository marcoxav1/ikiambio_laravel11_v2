...
@section('page_title','Editar — Vocab occurrence organismquantitytype')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab occurrence organismquantitytype #{ $item->oqtype_id }</h1>
...
<form method="POST" action="{{ route('vocab-occurrence-organism-quantity-type.update', $item) }}" class="card card-body">
  @csrf @method('PUT')
