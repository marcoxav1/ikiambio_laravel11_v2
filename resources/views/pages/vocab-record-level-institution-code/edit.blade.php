...
@section('page_title','Editar — Vocab record level institutioncode')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab record level institutioncode #{ $item->institutionCode_id }</h1>
...
<form method="POST" action="{{ route('vocab-record-level-institution-code.update', $item) }}" class="card card-body">
  @csrf @method('PUT')
