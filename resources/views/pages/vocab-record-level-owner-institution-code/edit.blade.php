...
@section('page_title','Editar — Vocab record level ownerinstitutioncode')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab record level ownerinstitutioncode #{ $item->ownerinstitutioncode_id }</h1>
...
<form method="POST" action="{{ route('vocab-record-level-owner-institution-code.update', $item) }}" class="card card-body">
  @csrf @method('PUT')

  <div class="form-grid">

    <div>
      <label class="label">Ownerinstitutioncode value *</label>
      <input type="text" name="ownerinstitutioncode_value" value="{{ old('ownerinstitutioncode_value', isset($item)? $item->ownerinstitutioncode_value : '') }}" class="input">
    </div>
  ...
  <div style="margin-top:12px;">
    ...
    <a href="{{ route('vocab-record-level-owner-institution-code.index') }}" class="btn">Cancelar</a>
  </div>
</form>
@endsection
