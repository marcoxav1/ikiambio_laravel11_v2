...
@section('page_title','Editar — Vocab occurrence reproductivecondition')

@section('content')
<h1 class="h4" style="margin:0 0 12px 0;">Editar — Vocab occurrence reproductivecondition #{ $item->reprocond_id }</h1>
...
<form method="POST" action="{{ route('vocab-occurrence-reproductive-condition.update', $item) }}" class="card card-body">
  @csrf @method('PUT')
